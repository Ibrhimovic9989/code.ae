  ...
  return (
    <div className={cn('flex flex-col gap-2', isUser ? 'items-end' : 'items-start')}>
      {hasBubbleContent ? (
        ...
        >
          {turn.text || (turn.pending ? <Spinner /> : null)}
        </div>
      ) : null}
        ...
        <div className="flex w-full max-w-[90%] flex-col gap-2">
          {turn.toolCalls.map((tc) => (
            <ToolCallCard
              key={tc.id}
              call={tc}
              labelCall={t('workspace.toolCall')}
              labelResult={t('workspace.toolResult')}
            />
          ))}
        </div>
      ) : null}
...
function ToolCallCard({
  ...
  return (
    <div className="rounded-lg border border-neutral-200 bg-neutral-50 text-xs dark:border-neutral-800 dark:bg-neutral-900">
      ...
      <pre
        dir="ltr"
        className="max-h-40 overflow-auto px-3 py-2 font-mono text-[11px] leading-tight text-neutral-700 dark:text-neutral-300"
      >
        {JSON.stringify(call.input, null, 2)}
      </pre>
      {call.status !== 'pending' ? (
        <details>
          <summary className="cursor-pointer border-t border-neutral-200 px-3 py-1.5 text-neutral-500 dark:border-neutral-800">
            {labelResult}
          </summary>
          <pre
            dir="ltr"
